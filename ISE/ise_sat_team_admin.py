...
DEFAULT_TIMEOUT = 60
DEFAULT_WORKERS = 4
USER_AGENT = "ise-sat-team-admin-automation/2.1"
...
<th>Profile Action</th><th>Rule Action</th><th>Generated IPs</th>
<th>Message</th>
</tr>
</thead>
<tbody>
...
def ensure_authorization_rule(
    ...
    rank_is_top = existing.get("rule", {}).get("rank") == 0

    if same and rank_is_top:
...
def process_host(
    ...
    try:
        ...
        log.info("%s | Authorization rule action: %s", host, rule_action)
...
def print_deployment_plan(results: list[HostResult]) -> None:
    ...
    for index, item in enumerate(results, start=1):
        ...
        print(f"  Rule action      : {item.rule_action or '-'}")
        print("  Generated IPs    :")
...
def main() -> int:
    ...
    created = sum(
        item.profile_action == "CREATED" or item.rule_action == "CREATED"
        for item in apply_results
    )
    updated = sum(
        item.profile_action == "UPDATED" or item.rule_action == "UPDATED"
        for item in apply_results
    )
    skipped = sum(
