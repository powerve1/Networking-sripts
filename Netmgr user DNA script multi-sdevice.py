...
def ssh_to_device_via_jump(jump_conn, device, device_user, device_pass):
    ...
    Returns:
        ...
        auth_failed (bool): True if password prompt repeated (bad creds)
    """
    ...
    for _ in range(6):
        ...
        if "password:" in tail:
            if password_sent:
                # We already tried once and got asked again -> auth failure
                auth_failed = True
                break
            output += jump_conn.send_command_timing(
                device_pass,
        ...
        # "Press RETURN" / "Press Enter" prompts
        if (
        ...
        # If we see a device prompt, we’re done with login
        if "#" in tail or ">" in tail:
            break

        # Nothing else relevant -> stop looping
        break

    if auth_failed:
        return output, False, True

    # Check if we ended at a device prompt
    ...
    while True:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if auth_failed:
                    msg = f"ERROR: Authentication failed for {device} (password prompt repeated)."
                    print(msg)
                ...
                # -----------------------------------------------------------------
                # Reached device prompt → push config
                # -----------------------------------------------------------------
                output += jump_conn.send_command_timing(
                    "terminal length 0",
                    strip_command=False,
                ...
                output += jump_conn.send_command_timing(
                    "username netmgr privilege 15 secret 9 $9$CPTwzB0jFadQYE$UgAW4K0Zxrmt0EhKKfhPXaligPN6ob43L13yw6jp./6",
                    strip_command=False,
                    strip_prompt=False,
                    delay_factor=1,
                )
                output += jump_conn.send_command_timing(
                    "enable secret 5 $1$5Eg2$wIQ2JMSqOhLj.UQgBc4Bs0",
                    strip_command=False,
                    strip_prompt=False,
                    delay_factor=1,
                )
...
def main():
    ...
    max_workers = 3           # process up to 3 devices in parallel
